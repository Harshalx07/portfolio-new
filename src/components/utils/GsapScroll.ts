...
export function setCharTimeline(
  ...
  character?.children.forEach((object: THREE.Object3D) => {
    const obj = object as THREE.Mesh & { material?: THREE.MeshStandardMaterial & { name?: string; transparent?: boolean; opacity?: number; color?: THREE.Color; emissive?: THREE.Color; emissiveIntensity?: number } };
    ...
    if (object.name === "screenlight") {
      ...
      gsap.timeline({ repeat: -1, repeatRefresh: true }).to(obj.material, {
        emissiveIntensity: () => intensity * 8,
        duration: () => Math.random() * 0.6,
        delay: () => Math.random() * 0.1,
      });
      screenLight = obj as unknown as THREE.Mesh;
    }
  });
